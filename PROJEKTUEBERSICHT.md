...
3. ~~Google~~ **lokal erledigt.** Cloud-Projekt `voulez-wallet`, Wallet API
   ...
   Projektrollen (Schlüssel in `certs/google-wallet-key.json`), Issuer-Konto
   „Voulez" — **Merchant ID `BCR2DN6D3KOMX735`, Issuer ID
   `3388000000023173458`**. Die beiden sind leicht zu verwechseln: die
   Merchant ID gehört zu Google Pay, in `GOOGLE_WALLET_ISSUER_ID` gehört die
...
4. **Noch offen bei Google:** Publishing-Zugang anfordern. Bis dahin
   ...
   Preview hat hier ohnehin keine Secrets, siehe §8. Das Dashboard meldet
   ausserdem „Geschäftliches Zahlungsprofil — Unvollständig"; für den
   Demo-Modus unerheblich, für die Freischaltung vermutlich nicht.
